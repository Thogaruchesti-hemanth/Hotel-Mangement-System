...
const firebaseConfig = {
    ...
    databaseURL: "https://hotel-management-system-902ff-default-rtdb.firebaseio.com",
...
function displayHotelInfo(hotel) {
    uniqueHotelId.textContent =hotel.hotelId;
...
function loadRooms(hotelId) {
    get(ref(db, `hotels/${hotelId}/rooms`)).then(snapshot => {
...
function addRoomToTable(room) {
    ...
    const checkInFormatted = room.checkIn ? new Date(room.checkIn).toLocaleString('en-GB',options) : 'N/A';
    const checkOutFormatted = room.checkOut ? new Date(room.checkOut).toLocaleString('en-GB',options) : 'N/A';
    row.innerHTML = `
...
function handleBookingFormSubmit(e) {
    ...
    const numberOfVisitors = parseInt(document.getElementById('number-of-visitors').value,10);
    ...
    if (checkInDate >= checkOutDate) {
     ...
     if (mobileNumber2 && !phoneRegex.test(mobileNumber2)) { // Allow second number to be optional
    ...
    if (aadhaarScanFile) {
        ...
        const aadhaarScanStorageRef = storageRef(storage, `aadhaarScans/${uniqueCode}`);
        aadhaarScanPromise = uploadBytes(aadhaarScanStorageRef, aadhaarScanFile).then(() => {
            return getDownloadURL(aadhaarScanStorageRef);
        });
    } else {
        // Use existing Aadhaar scan if no new file is uploaded
        aadhaarScanPromise = get(ref(db, `hotels/${hotelId}/rooms/${roomNumber - 1}/aadhaarScan`)).then(snapshot => snapshot.val());
    }

    aadhaarScanPromise.then(aadhaarScanURL => {
...
// Utility function to generate a unique code
        ...
        if (roomNumber) {
            ...
            get(roomRef).then(snapshot => {
                ...
                if (room && room.status === 'Booked') {
                    ...
                    document.getElementById('aadhaar-number').value = room.aadhaarNumber || '';
                    document.getElementById('mobile-number1').value = room.mobileNumber1 || '';
                    document.getElementById('mobile-number2').value = room.mobileNumber2 || '';
                    document.getElementById('purpose').value = room.purpose || '';
                    document.getElementById('advance-amount').value = room.advanceAmount || '';
                    document.getElementById('number-of-visitors').value = room.numberOfVisitors || '';
                    ...
                    document.getElementById('booking-form').onsubmit = function(e) {
                        ...
                        const updatedGuestName = document.getElementById('guest-name').value;
                        ...
                        const updatedCheckOut = document.getElementById('check-out').value;
                        const updatedAadhaarNumber = document.getElementById('aadhaar-number').value;
                        const updatedMobileNumber1 = document.getElementById('mobile-number1').value;
                        const updatedMobileNumber2 = document.getElementById('mobile-number2').value;
                        const updatedPurpose = document.getElementById('purpose').value;
                        const updatedAdvanceAmount = document.getElementById('advance-amount').value;
                        const updateNumberOfVisitors = parseInt(document.getElementById('number-of-visitors').value, 10);
                        ...
                        if (updatedMobileNumber2 && !phoneRegex.test(updatedMobileNumber2)) { // Allow second number to be optional
                            alert('Mobile Number 2 must be a 10-digit numeric value.');
                            return;
                        }
...
document.getElementById('checkout-room-btn').addEventListener('click', handleCheckout);
    ...
    if (roomNumber) {
        ...
        get(roomRef).then(snapshot => {
            ...
            if (room && room.status === 'Booked') {
                ...
                // Close button logic
                document.getElementById('close-checkout-popup-btn').addEventListener('click', function() {
                ...
                document.getElementById('checkout-form').onsubmit = function(e) {
                  ...
                  fetch('https://script.google.com/macros/s/AKfycbyl3f1olwXv-pPld_Jrdo9hCHK0NG3Gpnkm9lQMW6DsP8w_U-HdJFkSC9x55_Pqh1oA/exec', {
                    method: 'POST',
