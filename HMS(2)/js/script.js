...
function addRoomToTable(room) {
    ...
    const checkInFormatted = room.checkIn ? new Date(room.checkIn).toLocaleString() : 'N/A';
    const checkOutFormatted = room.checkOut ? new Date(room.checkOut).toLocaleString() : 'N/A';
    row.innerHTML = `
    ...
    aadhaarScanPromise.then(aadhaarScanURL => {
        ...
        get(roomRef).then(snapshot => {
            ...
            if (room.status === 'Available') {
                const checkIn = new Date().toLocaleDateString("en-IN", { timeZone: "Asia/Kolkata" });
        ...
        get(roomRef).then(snapshot => {
            ...
            if (room && room.status === 'Booked') {
                ...
                document.getElementById('checkout-form').onsubmit = function(e) {
                  ...
                  fetch('https://script.google.com/macros/s/AKfycbxFL___kUO8BBwRbXp4hEOmGG6GGEtSIjtSdIKcUPo/dev', {
                    method: 'POST',
